        ...
        <form onSubmit={handleFormSubmit} className="space-y-4" noValidate>
          {isRegister && (
            <div className="space-y-1">
              <label className="block text-xs font-bold uppercase tracking-wider text-[#7a6255] px-1">Pseudo</label>
              <input type="text" value={pseudo} onChange={(e) => setPseudo(e.target.value)} placeholder="Ton pseudo" className="w-full rounded-[1.2rem] border-[2px] border-[#d7b59d] bg-[#fcf9f6] px-4 py-3 text-[#5d4d44] placeholder-[#a68e81] shadow-inner focus:outline-none focus:ring-2 focus:ring-[#e59a86] transition-all" />
            </div>
          )}

          <div className="space-y-1">
            <label className="block text-xs font-bold uppercase tracking-wider text-[#7a6255] px-1">Email</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="toi@exemple.com" className="w-full rounded-[1.2rem] border-[2px] border-[#d7b59d] bg-[#fcf9f6] px-4 py-3 text-[#5d4d44] placeholder-[#a68e81] shadow-inner focus:outline-none focus:ring-2 focus:ring-[#e59a86] transition-all" />
          </div>

          <div className="space-y-1">
            <div className="flex justify-between items-center px-1">
              <label className="block text-xs font-bold uppercase tracking-wider text-[#7a6255]">Mot de passe</label>
              {!isRegister && (
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full rounded-[1.2rem] border-[2px] border-[#d7b59d] bg-[#fcf9f6] px-4 py-3 text-[#5d4d44] placeholder-[#a68e81] shadow-inner focus:outline-none focus:ring-2 focus:ring-[#e59a86] transition-all" />
          </div>

          {isRegister && (
            <div className="space-y-1">
              <label className="block text-xs font-bold uppercase tracking-wider text-[#7a6255] px-1">Confirmer le mot de passe</label>
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" className="w-full rounded-[1.2rem] border-[2px] border-[#d7b59d] bg-[#fcf9f6] px-4 py-3 text-[#5d4d44] placeholder-[#a68e81] shadow-inner focus:outline-none focus:ring-2 focus:ring-[#e59a86] transition-all" />
            </div>
          )}

          <button type="submit" disabled={isLoading} className="w-full mt-2 rounded-[1.25rem] border-[3px] border-[#e4a58f] bg-[#e59a86] py-3.5 text-center text-[1rem] font-bold text-[#fff8f5] shadow-[0_4px_0_rgba(171,98,77,0.85)] transition-all hover:translate-y-[2px] hover:shadow-[0_2px_0_rgba(171,98,77,0.85)] active:translate-y-[4px] active:shadow-none focus:outline-none focus:ring-2 focus:ring-[#5b473d] disabled:opacity-70">
